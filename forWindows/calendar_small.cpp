...
int main(){
    ...
    // cout << fName << endl; //debug 生成したファイル名確認用
...
void printMonthYear(Pos pos1, Pos pos2, const tm* now, RGB rgb, ofstream& fout){ //位置の変数化は？
    /* printMonth */
    const uint size = 60; //font-size
    string str = "<tspan text-anchor=\"end\">" + to_string(now->tm_mon + 1) + "月</tspan>";
    text(Pos{pos1.x + size * 2.5, pos1.y - size}, str, rgb, size, fout); //注意:右寄せ
    /* printYear */
...
// cout << resetiosflags(ios_base::floatfield); //使えなくね…？